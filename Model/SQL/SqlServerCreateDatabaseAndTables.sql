...
IF EXISTS (SELECT name FROM sys.databases WHERE name = 'shopping_model')
DROP DATABASE [shopping_model]
...
SET @sql = 
  N'CREATE DATABASE [shopping_model]
	ON PRIMARY ( NAME = shopping_model, FILENAME = "' + @Default_DB_Path + N'shopping_model.mdf")
	LOG ON ( NAME = shopping_model_log, FILENAME = "' + @Default_DB_Path + N'shopping_model_log.ldf")'

EXEC(@sql)
PRINT N'Database [shopping_model] created.'
GO
